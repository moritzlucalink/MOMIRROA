...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 11:50:09 2024
...
"""
problem instance (P3) with k=4 and n=2 from
...
def build_model(m):
    ...
    model.cons1 = Constraint(expr = model.x5**2 + model.x6**2 - 9 <= 0)

    # define objectives
    model.objective0 = Objective(expr = model.x1 + model.x2 + model.x5)
    model.objective1 = Objective(expr = model.x3 + model.x4 + model.x6)

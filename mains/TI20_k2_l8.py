...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 10 13:07:18 2024
...
"""
problem instance (P3) with k=2 and l=8 from
...
def build_model(m):
    ...
    model.cons1 = Constraint(expr = model.x3**2 + model.x4**2 + model.x5**2\
                                  + model.x6**2 + model.x7**2 + model.x8**2\
                                  + model.x9**2 + model.x10**2 - 9 <= 0)

    # define objectives
    model.objective0 = Objective(expr = model.x1 + model.x3 + model.x4 \
                                      + model.x5 + model.x6)
    model.objective1 = Objective(expr = model.x2 + model.x7 + model.x8 \
                                      + model.x9 + model.x10)

...
"""
problem instance (P3) with k=8 and n=8 from
...
def build_model(m):
    ...
    model.x15 = Var(within=Integers, bounds=(-3,3))
    model.x16 = Var(within=Integers, bounds=(-3,3))

    # define constraints
    model.cons0 = Constraint(expr = -model.x1**2 - model.x2**2 - model.x3**2\
				  - model.x4**2 - model.x5**2 - model.x6**2\
				  - model.x7**2 - model.x8**2 + 1 <= 0)
    model.cons1 = Constraint(expr = model.x9**2 + model.x10**2 + model.x11**2\
                                  + model.x12**2 + model.x13**2 + model.x14**2\
                                  + model.x15**2 + model.x16**2 - 9 <= 0)

    # define objectives
    model.objective0 = Objective(expr = model.x1 + model.x2 + model.x3 \
				      + model.x4 + model.x9 + model.x10 \
                                      + model.x11 + model.x12)
    model.objective1 = Objective(expr = model.x5 + model.x6 + model.x7 \
                                      + model.x8 + model.x13 + model.x14 \
                                      + model.x15 + model.x16)
...
options.soft_utopian_check = False
options.tight_image_box = False
options.refine_until_feasible_search = 4
options.enforced_BT = True

options.OBBT_timelimit = 1
options.OBBT_gap = 0.05

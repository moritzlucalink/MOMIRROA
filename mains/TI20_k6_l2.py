...
"""
problem instance (P3) with k=6 and l=2 from
...
def build_model(m):
    ...
    model.cons0 = Constraint(expr = -model.x1**2 - model.x2**2 - \
                                    model.x3**2 - model.x4**2 - model.x5**2 \
                                        - model.x6**2 + 1 <= 0)
    model.cons1 = Constraint(expr = model.x7**2 + model.x8**2 - 9 <= 0)

    # define objectives
    model.objective0 = Objective(expr = model.x1 + model.x2 + model.x3 + model.x7)
    model.objective1 = Objective(expr = model.x4 + model.x5 + model.x6 + model.x8)

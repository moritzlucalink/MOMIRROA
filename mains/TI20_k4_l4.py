...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 11:58:00 2024
...
"""
problem instance (P3) with k=4 and n=4 from
...
def build_model(m):
    ...
    model.x5 = Var(within=Integers, bounds=(-3,3))
    model.x6 = Var(within=Integers, bounds=(-3,3))
    model.x7 = Var(within=Integers, bounds=(-3,3))
    model.x8 = Var(within=Integers, bounds=(-3,3))

    # define constraints
    model.cons0 = Constraint(expr = -model.x1**2 - model.x2**2 - \
                                    model.x3**2 - model.x4**2 + 1 <= 0)
    model.cons1 = Constraint(expr = model.x5**2 + model.x6**2 + \
                                    model.x7**2 + model.x8**2 - 9 <= 0)

    # define objectives
    model.objective0 = Objective(expr = model.x1 + model.x2 + model.x5 + model.x6)
    model.objective1 = Objective(expr = model.x3 + model.x4 + model.x7 + model.x8)
...
options.nlp_feasibility_only = True
